#!/usr/bin/env node
const fs = require("fs");
//fs.writeFileSync("abc.txt", "what you doing");
...
function wcat(arguments) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        } else if (options.includes("-n") && !options.includes("-b") ||
            (options.includes("-n") && options.includes("-b") && options.indexOf("-n") < options.indexOf("-b"))) {
            let lines = data.split("\n");
            for (let j = 0; j < lines.length; j++) {
                console.log(numbering + ". " + lines[j]);
                numbering += 1;
